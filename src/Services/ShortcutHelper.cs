...
///
/// A second, easily-missed step is required too: telling the *currently running process*
/// to identify as that AUMID. Windows normally only picks up the shortcut's AUMID when the
/// app is launched by double-clicking the shortcut itself; if it's launched any other way
/// (from Visual Studio, a different shortcut, pinned taskbar icon before the shortcut
/// existed, etc.) toasts fail completely silently - no exception, no toast, nothing.
/// Calling SetCurrentProcessExplicitAppUserModelID fixes that for every launch path.
/// </summary>
public static class ShortcutHelper
{
    ...
    /// <summary>Call once at startup, before showing any toast. Binds the running process to
    /// the app's AUMID and makes sure the Start Menu shortcut exists with the same AUMID.</summary>
    public static void EnsureAppIdentity()
    {
        try
        {
        ...
        {
            // Best effort - older Windows builds still fall back to the shortcut-based AUMID.
        }
    ...
    public static void EnsureShortcutWithAumid()
    {
        ...
        {
            // Non-fatal: toasts may fail to appear, but the app still runs.
        }
    }
